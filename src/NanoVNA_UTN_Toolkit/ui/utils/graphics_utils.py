...
def create_left_panel(S_data, freqs, graph_type="Diagrama de Smith", s_param="S11"):
    ...
    # --- Inicializamos cursor_graph para scope ---
    cursor_graph = None
    ...
    if graph_type == "Diagrama de Smith":
        ...
        cursor_graph, = ax.plot([], [], 'ro', markersize=6)

    elif graph_type == "Módulo":
        ...
        ax.plot(freqs*1e-6, np.abs(S_data), 'b.-')
        ...
        ax.plot(freqs*1e-6, np.angle(S_data, deg=True), 'b.-')
        ax.set_xlabel("Frequency [MHz]")
